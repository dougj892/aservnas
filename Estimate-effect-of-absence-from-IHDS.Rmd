...
$$ newweight = oldweight*\frac{25-daysabsentpermonth}{25} $$ 
...
state_averages <- ihds %>% filter(!is.na(TA8B) & !is.na(CS13)) %>% 
  ...
  mutate(new.weight = WT*(25-CS13)/25) %>%
  group_by(State) %>% 
...
# Create variable for difference in rank
state_averages <- state_averages %>% mutate(rank_diff = factor(full_rank - absence_rank))

# Create scatter plot of full sample ASER4 score versus absence weighted score for each state
ggplot(state_averages, aes(x = full_sample, y = absence_weighted, label = State, colour = rank_diff)) +
  geom_abline(intercept = 0, slope = 1, color="orange")  +
  geom_label_repel(size = 3) +
  geom_point() +
  scale_colour_manual(values = c("darkred", "red", "black", "blue", "darkblue")) +
  guides(colour = guide_legend(override.aes = list(size = 5)))+
  # theme(legenid.key=element_rect(fill = "black")) +
  labs(x = "IHDS score", y ="Expected absence-weighted IHDS score", colour = "Change in rank")
